...
def otp(phoneNumber):
    ...
    payload = f"variables_values={otp_number}&route=otp&numbers={phoneNumber}"
    headers = {
...
def otp_validate(request,otp):
    print(otp)
    if request.method == 'POST':
        user_otp = request.POST['user_otp']
        if user_otp == otp:
            user=Account.objects.create_user(first_name=otphelper.firstname,last_name=otphelper.lastname,username=otphelper.username,email=otphelper.email,password=otphelper.password)
            user.phone_number=otphelper.phone
            user.is_active = True
            user.save()
...
def register(request):
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            otp_number = otp(phoneNumber=phone_number)
            otphelper.username = username
            otphelper.password = password
            otphelper.email = email
            otphelper.firstname = first_name
            otphelper.lastname = last_name
            otphelper.phone = phone_number

            return redirect('otp_validate',otp=otp_number)
